...
- **A 4Ws note is an invitation, not a verdict, and it is a question.** Wherever WANT, WAY or WHO/WHEN did not land, the note gives the question to ask next time, about this issue, in the team's own words: "What would enough water pressure for Raw look like?", "Which one of these do we try first?", "Who takes this, and when do we look at it again?". Not "the room didn't pause", not "an opportunity to name what better looks like", and never that a step was missed. A note that does not end in a question mark is wrong.
