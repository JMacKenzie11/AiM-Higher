...
- **AI clarity scoring on every commitment** — a background Anthropic Haiku call (`src/lib/commitments/clarity.ts`, model `ANTHROPIC_CLARITY_MODEL`) scores two boolean criteria (`clarity_timeline`, `clarity_success`) + an optional `clarity_note` at creation time (whether the commitment was hand-typed OR extracted from a meeting). Best-effort — a failed API call never blocks the save; the row just shows an unassessed dot until re-scored. Rendered as a colored dot beside the resolve circle (green = both criteria met, amber = at least one not, muted = unassessed). Editor allows manual override and hides the refinement-note field when both criteria are already YES.
