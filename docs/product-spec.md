...
- **Every commitment on an issue is the same kind of thing.** The row's three commitment columns are occupied by a *region* laid out on CSS `subgrid`, and each commitment is a line inside it — description, owner, due date — aligned to the COMMITMENT / ASSIGNED TO / DUE DATE headers however many there are. The add form is the last line, always present, so a first and a fifth commitment are entered the same way.
  ...
  - **Commitments can be closed from the issue.** Each open line carries a check control that calls `markKeptAction`. Previously there was no way to resolve an issue commitment on this page at all, which sent people to `/commitments` to finish work they were looking straight at.
  - **The clarity dot moved onto the line, and then to the end of it.** Clarity belongs to a commitment, and an issue has any number of them, so the row's leading clarity column is gone. It first landed next to the close control, which put two circles in front of every description and made it ambiguous which one resolved the commitment; it now trails the text as the annotation it is, leaving one control at the head of the line.
