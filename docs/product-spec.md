...
- **Registry** — `src/lib/practices/registry.ts`. Adding an agent is a registry entry plus a prompt file at `prompts/practices/<id>.md`; no other code change unless the agent needs a new output card component. Each entry declares:
  - `basePromptMode` (`full_coach` or `voice_only`). `full_coach` splices `prompts/aims-voice.md` into `prompts/leadership-coach.md` (the current three agents); `voice_only` loads `aims-voice.md` alone as the base — no coaching spine, no diagnostic modes, no patterns-to-watch-for. Used by structural agents (Functional Chart Builder) where the agent prompt IS the flow.
  - `skipSetup` — when true, the partner picker + setup step are bypassed; the conversation opens directly in the chat.
  - `scriptedOpener` — optional string persisted as the first assistant message with ZERO API calls when the agent is attached. Kept in the registry so the opener is versioned alongside the prompt.
  - `firstTurn` — `"scripted"` (uses `scriptedOpener` verbatim, no API), `"generate"` (calls the model with the agent's prompt and a synthetic "introduce yourself and start" instruction — streams a fresh opener each time), or omitted (no opener). Existing agents ship `"scripted"` for backward compat.
...
- **Two paths to attach an agent:**
  1. **In-thread AgentPicker** — the composer's Agent pill on any owner-general chat opens `AgentPicker` (categories + rows, same shape the retired PracticeCards had). Selecting an agent fires `setConversationAgentAction`, which validates ownership, refuses if any user-role message exists (lock), wipes any prior assistant opener, sets `practice_id`, and — for `firstTurn: "scripted"` agents — inserts the scripted opener inline. For `firstTurn: "generate"` agents, the client fires `/api/coach` with `generateOpener: true` immediately after, streaming a fresh opener into a new assistant bubble.
