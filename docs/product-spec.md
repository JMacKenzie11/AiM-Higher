...
- **Commitments live UNDER the issue, and each one is a real `CommitmentRow`** (2026-09-14). The issue's own grid is five columns — drag | issue | what we want | delete | resolve — and the commitments region spans `grid-column: 1 / -1`, so auto-placement drops it onto the row beneath rather than beside the title.
  ...
  - **The add line's plus is a `<label>`, not an icon.** Filled in `--primary`, it reads as a button, so it behaves like one: clicking focuses the textarea. A label rather than a button because the form is the textarea, and this adds no second tab stop; its visually-hidden text is also what gives that textarea an accessible name. The circle uses `--primary` / `--on-primary` rather than the raw brand hexes because the dark theme redefines the pair together.
