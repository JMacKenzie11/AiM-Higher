...
- **Scope-in (system_admin + aims_guide):** pressing a company control — the fleet list, Guide HQ's caseload and attention list, the admin dashboard's at-risk and activity lists — invokes the `scopeIntoCompany` server action, which authorizes the caller, sets the scope cookie, and returns where to land. The Sidebar renders a persistent "SYSTEM ADMIN · COMPANY NAME" (or "AIMS GUIDE · COMPANY NAME") context pill under the logo with an *Exit company* affordance in the user menu at the bottom of the rail.
  ...
  - **Role restriction.** The cookie is only ever written for `system_admin` and `aims_guide`, and the redirect above applies only to them. Company users never read it and are never redirected; the resolver returns `profile.company_id` first.
