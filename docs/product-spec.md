...
## 7. AiMS Implementation Status (discipline maturity)

Company-wide "how are we doing at running the AiMS disciplines?" view at `/scorecard`. Visible to **everyone in the company** (transparency by design — team members see the same view leaders do). Nav label: **AiMS Implementation Status**; internal scorer/API names retain `scorecard` for stability.
