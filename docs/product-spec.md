...
- **Roles:**
  ...
  - `portfolio_admin` — a PE operating partner who owns this instance's portfolio of companies. **Instance-wide READ on company content, and a closed list of three administrative writes on the container around it.** Belongs to no company, like `system_admin` and `aims_guide`; unlike `aims_guide` there is NO grant table, so reach extends automatically to companies created after the role was granted. Assignable only by a `system_admin`, from the platform dashboard. Migrations 0190-0192.
    - **Reads** every company's content: commitments, priorities, SFAs, goals, meetings, measures, issues, foundation, chart, scorecards. Forty-nine permissive SELECT policies added beside the existing ones, never editing them, so nothing any other role sees changed. Also reads `company_feature_events` (they manage packaging); company admins remain without that read, per the billing-adjacent reasoning in 0173.
    - **Does not read**: private coaching conversations, messages, shares and analyses (the owner and explicit shares only, as for every other role); personal notifications; a guide's own session briefs; OAuth credentials; guide assignments; platform cost and ops telemetry; the audit logs. Published Classroom content IS readable, so a scoped-in portfolio admin sees the same shelves a member does.
    - **Does not see rows belonging to no tenant.** An unrouted meeting, a company-less profile, an assessment taken outside a company — "every company on the instance" does not include "and the rows that are in none of them". This is hazard 1's exact shape and is probed with a seeded unrouted row and a `system_admin` control that can see it. Their own profile row stays visible through the ordinary self-read, and that carve-out is asserted rather than ignored.
    - **Writes exactly three things**, and the list is closed: (1) create a company; (2) manage its settings and feature flags; (3) invite users into company-scoped roles, including a company's first `company_admin`. **Archive yes, delete no** — `deleteCompanyAction` stays `system_admin` only, and `deleted_at` is absent from the column allowlist on `companies_restrict_admin_columns`, so the column is unreachable for the role even if the action were widened by mistake.
    - **Can never create another `portfolio_admin` or a `system_admin`.** Four independent places say so: the invite action's role ceiling, `updateUserAction`'s grant guard, the `role in ('company_admin','team_member')` clause on `profiles_insert_portfolio`, and the fact that both platform-role actions are `system_admin`-only. The harness probes the SQL one as a real `portfolio_admin` JWT, because it is the only one an app-layer mistake cannot reach past.
    - **Creating a company needs no content grant.** `createCompany` seeds chart roots and an opening quarter — writes to `functions` and `quarters` — through `seed_company_roots()`, a SECURITY DEFINER function that decides for itself who may call it. Without it, making the button work would have required a `portfolio_admin` INSERT policy on two content tables, which is precisely the hole the closed list exists to prevent.
    - **Write affordances are off everywhere else, without a single content page being changed.** Every content surface computes its edit affordance from `isAdminForCompany` or `canWriteOwnedRow`, and neither admits this role. The access gates that needed widening use `canViewCompany` instead. RLS is the enforcement; the UI is the courtesy, and here the two agree because both derive from the same helper.
    - **Enforced by a static check, not by convention.** `npm run rls:hazards` fails if a write policy names `portfolio_admin` on any table outside `companies`, `company_features`, `profiles` and `portfolio_admin_events` — matching both the `is_portfolio_admin()` idiom and a bare `auth_role() = 'portfolio_admin'`. The check plants a deliberately wrong policy on `commitments` inside a rolled-back transaction on every run and fails if it is not caught, so a clean result is never the silence of a broken matcher.
    - **Every scope-in and every administrative action is recorded** in `portfolio_admin_events`: actor, action, company, timestamp, detail. Append-only — `system_admin` SELECT only, no UPDATE or DELETE policy for anyone, and the INSERT policy admits only a `portfolio_admin` writing a row about themselves. **This is the accountability layer that replaces per-company grants:** with no assignment table there is no row to inspect after the fact, so this is what exists instead. Written by the action layer rather than a trigger, because two of the recorded actions ("scoped in", "invited a user") are not row changes a trigger can see.
  - `team_member` — read + self-write.
- **Scope-in (system_admin, aims_guide, portfolio_admin):** pressing a company control — the fleet list, Guide HQ's caseload and attention list, the admin dashboard's at-risk and activity lists — invokes the `scopeIntoCompany` server action, which authorizes the caller, sets the scope cookie, and returns where to land. The Sidebar renders a persistent "SYSTEM ADMIN · COMPANY NAME" (or "AIMS GUIDE · COMPANY NAME") context pill under the logo with an *Exit company* affordance in the user menu at the bottom of the rail.
  - **The scope cookie is bound to the profile it was issued for** (`<profileId>:<companyId>`, 2026-09-14). A cookie belonging to another user resolves to nothing, whatever created the current session. **Every reader goes through `scopedCompanyIdForProfile`** — middleware included. It did not at first: the binding landed with `getScopedCompanyId` updated and `middleware.ts` still comparing the whole bound value against a bare company id from the URL, which can never match, so every cross-tenant caller was bounced to the scope picker on every `/admin/companies/<id>` visit — including immediately after pressing *Settings*, which had just scoped them into that company. A format is a contract between writers and readers, and the guard that existed covered only writers. Found on production: a newly created `portfolio_admin` signed in and landed inside a company's dashboard having never pressed a scope-in control, with no row in `portfolio_admin_events` to say they had entered. The cookie was the previous occupant's — `path=/`, eight hours, and cleared by exactly two code paths, neither of which was signing out or accepting an invite (which creates a session for a *different* user in the same browser). Adding more `clear()` calls would have closed those two doors; binding closes the class. Sign-out and invite-acceptance now clear it as well, as hygiene rather than as the protection.
  - **Scope-in is never a GET side effect.** *Two exceptions existed and both are gone.* Middleware's write on `GET /admin/companies/[id]` was removed first (see below). `/api/coach/align-scope` was the second: a route handler that rewrote the cookie so a coaching chat would render against its own tenant, which meant reading one conversation silently moved an operator's scope — and their Dashboard link — to that company for the next eight hours. Removed 2026-09-14. The two chat pages now pass the conversation's own `company_id` to the one thing that needed it (the agent gate), which is the more correct argument anyway: the question is whether the caller may run a practice on *this chat's* company, not on whichever one their cookie names. A source-level test (`lib/admin/scope-writers.test.ts`) asserts that the only writers of the cookie are the two server actions, and that no route handler, page, or middleware file contains the call. No request changes who the caller is acting as; only the action does. Every scope-in control is a `<button>` (`ScopeIntoCompanyButton`), never a `<Link>`, so nothing can be prefetched, hovered or crawled into firing.
    - *Why this is not header detection any more.* Middleware used to write the cookie on `GET /admin/companies/[id]` and skip the write for requests carrying `Next-Router-Prefetch`, `Purpose: prefetch` or `Sec-Purpose: prefetch`. That guard did not hold: Next strips `next-router-prefetch` before middleware sees it, so an app-router prefetch was indistinguishable from a real navigation, and a `<Link>` scrolling into view on `/hq` could move the operator into a company nobody chose. Adding `prefetch={false}` to every link closed it, but only for as long as everyone remembered to write it. Removing the write removes the class of bug.
  - **Unscoped company URLs.** A `system_admin` or `aims_guide` who requests `/admin/companies/[id]` while scoped elsewhere, or nowhere, is redirected to Guide HQ to choose. Deep links keep working unchanged for anyone already scoped into that company. Decision logic is pure and unit-tested in `src/lib/admin/scope-request.ts`.
...
- **Live compute + weekly snapshot.** The current score is computed **live** on every page load (six small reads, no cache) so behavior changes show up immediately. A weekly Sunday cron (`/api/cron/scorecard`) writes one row per (company, snapshot_date, discipline) to `company_discipline_snapshots` for the historical trend line.
  - **One weekly run produces two `snapshot_date` values, and that is correct.** The cron fires at 07:00 UTC Sunday but `snapshot_date` is stamped as today in each *company's* timezone, so western tenants (America/Anchorage, America/Denver) are still on Saturday when the row is written. A week's snapshots therefore appear in the data as a Saturday/Sunday pair, not as one date. Reading per-company rather than per-date is the intended shape.
...
- **Execution tile excludes soft-deleted and parked commitments** (fixed 2026-09-14). It was the only follow-through path in the product that did not: `company_follow_through` (0174), `computeQuarterKeepRate` and the dashboard's quarter query all filter both. Deleting a commitment is meant to remove it from every list, count and metric; on this tile it kept costing the company 0.5 points a week indefinitely, because the aging query has no window for a row to age out of.
  - **Measured before the fix**, on production: 31 soft-deleted commitments counted as aging across five companies, and one deleted row sat in a follow-through numerator. **Four of eight companies were scoring below what they had earned** — Benson Seafood 7.5 against 10.0, Geo-Sci 7.0 against 10.0, Centre North 7.0 against 9.0, Howard Concrete 7.5 against 9.0. B&B Electric showed no delta only because both counts exceeded the 3-point aging cap. Parked rows contributed nothing: two exist and neither was aging.
  - **Fixed forward only; stored history was deliberately left alone.** The 34 execution snapshots covering ten weeks from 2026-08-15 keep the numbers they were written with. Recomputing them would rewrite figures clients have already been shown; leaving them means **the trend line steps up once, on 2026-09-14, for a reason no client behaviour caused**. That is the decision, recorded here so the step has a written cause. `compareOverall` restricts to disciplines both points scored, so the step cannot present as a false *drop* on Guide HQ.
  - **Still open:** the open-commitments query has no date window, so a commitment from 2024 still costs 0.5 points today. Every other commitment read in the product is bounded. Narrowing it would move scores again and is its own decision.
...
- **Issue-linked commitments appear on `/commitments` too** (2026-09-14). They were filtered out of all three of that page's queries on the grounds that they had their own home here. That made `/commitments` an incomplete ledger: those commitments have always counted in the follow-through rate (`company_follow_through` filters only deleted and parked), in the weekly scorecard (`maturity/scorers/execution.ts`) and in the coach's context, so the page people actually **read** was the only place they did not appear — somebody could be judged on work they could not see listed. Each carries a **From issue** pill **in the Priority column**, linking to `/issues#issue-<id>`; there is no issue detail page, so the row itself is the anchor. It sits there rather than beside the description for two reasons: an issue-linked commitment has no priority — an issue is neither a priority nor attached to one — so the link chip's usual behaviour of labelling itself with whatever the commitment is linked to would put the issue's title under a header reading PRIORITY; and a pill below the description made that cell taller than its neighbours and pushed the row out of alignment. There is no link picker on those rows: moving a commitment off its issue belongs in the issue's own context.
  - **They classify as "operational" under the Type filter**, because that filter means "carries no priority" rather than "carries no link". Consistent with the Priority column showing the *From issue* pill rather than a priority. Functional-area-linked commitments have sat in that bucket since `0143`. Whether either belongs there is a question about what the word promises, and it is not answered by this change.
- **Commitments live UNDER the issue, and each one is a real `CommitmentRow`** (2026-09-14). The issue's own grid is five columns — drag | issue | what we want | delete | resolve — and the commitments region spans `grid-column: 1 / -1`, so auto-placement drops it onto the row beneath rather than beside the title.
  - **Why it moved.** The region used to sit at `grid-column: 4 / 7`, which made the issue's row double as the *first* commitment's row. That put an issue-level *Resolve* button inches from a commitment-level resolve control, with nothing to distinguish which scope you were acting on. Reported as "super confusing", and it was: the two controls were adjacent, similar, and did different things to different objects.
  - **`CommitmentRow` is reused, not re-implemented.** `/issues` is now its fourth consumer alongside `/commitments`, Guide HQ (`companyLabel`) and the priority detail pages (`hidePriority`). Parity is therefore structural rather than maintained by hand: the resolve circle opens a menu and never resolves on click, reschedule / missed-reason / late-keep open as full-width strips, the 30-second undo chip is the same chip, and delete sits where it always does. Passed `hidePriority` (an issue-linked commitment has no priority) and `canLink={false}` (moving a commitment off its issue belongs in the issue's context, not a chip menu).
  - **Five components were deleted from `IssueCard`,** not adapted: `OpenCommitmentLine`, `DoneCommitmentLine`, `CommitmentDescriptionEditor`, `OwnerAssignmentEditor` and `DueDateEditor`. They were a parallel, thinner implementation of the same interactions and had already drifted: the close control resolved on a single click with no menu and no undo, and `DueDateEditor`'s Save / Cancel buttons rendered inside a 110px cell using `styles.resolveButton` — **literally the same class as the issue's Resolve button**, which is why they read as issue-level controls.
  - **The commitments band is full-bleed, and that is an alignment fix rather than a style choice.** Measured from the card's content edge, the issue title sat at 132px and the commitment descriptions at 161px, because the panel's own horizontal padding stacked on top of the padding `.row` already carries. Twenty-nine pixels apart reads as a wonky indent rather than a deliberate one. The band cancels `.issueRow`'s padding with a negative margin and carries none of its own, so `.row`'s 16px lands the commitment grid exactly on the issue grid: circle over circle, trash over trash, drag handle over the clarity dot, title over the descriptions it heads. Full-bleed corners cannot be rounded without the fill pulling away from the card edge, so it is a band, not an inset card.
  - **`.rowResolved` dims the CELLS, not the row.** `opacity` below 1 creates a stacking context, so dimming the row trapped the resolve menu's `z-index: 20` inside it: the menu painted UNDER the following row and inherited the row's 60% opacity — reported as "translucent and doesn't seem like it's over the underlying text". Moving the dim down one level (`.rowResolved > *:not(.resolveMenu)`) leaves the row without a stacking context and excludes the menu from the fade.
  - **The issue title is semibold.** It and the commitment descriptions under it were both plain `--text-body`, which made a parent and its children the same weight and left the card with no hierarchy to read. The recessed band separates them; the weight says which one is the heading.
  - **The resolved row's DOM order matches its column order too.** It was emitted check-delete-placeholder against a placeholder-check-delete column order, which put that list's title and outcome on a second line under the controls — the same defect as the open row, fixed in the open list first and left standing here.
  - **The issue row's DOM order matches its column order** (drag, resolve, delete). They are placed explicitly at columns 1-3 and grid auto-flow only moves forward, so emitting them resolve-delete-drag left the drag handle unable to reach column 1 on a row already past it: it started a SECOND row and took the title and outcome with it.
  - **`CommitmentSubHeader` places its four labels explicitly** rather than rendering eight cells and leaning on `:nth-child(6)` to hide the priority slot. Once that rule became class-based, nothing hid the sixth cell, so eight cells went into seven columns and STATUS wrapped onto a line of its own.
  - **The resolve menu renders LAST among the row's children, and that placement is load-bearing.** It is `position: absolute`, anchored to `.row`, so its DOM position changes nothing visually — but it is a direct child, and several rules address these children BY POSITION: `.rowNoPriority` hid the priority slot with `:nth-child(6)`, and the mobile breakpoint maps `.row > :nth-child(1..8)` onto grid areas. Rendered second, it shifted every later child by one whenever the menu was open, so opening the circle menu hid the **owner** instead of the priority placeholder and remapped the whole mobile layout. Pre-existing, and invisible on `/commitments` because that page does not pass `hidePriority`; `/issues` and the priority detail pages are the surfaces that do. The priority slot is now also hidden by a class (`.priorityHidden`) rather than by counting, so a future conditional child cannot recreate it.
  - **The add line places every cell with an explicit `grid-column`.** It briefly leaned on `.rowNoPriority`'s `:nth-child(6)` the way `CommitmentRow` does, but `:nth-child` counts every element child and this form carries three `<input type="hidden">` before its first cell. The count was off by three, so the rule hid the wrong element, the date landed in the STATUS column and `Add` wrapped onto a row of its own.
  - **The drag handle is column 1, ahead of the issue's own check and trash.** It is an issue-only control, so the commitments band carries a left pad of exactly its width plus one gap, which keeps the commitment grid aligned under the issue grid: the issue's check sits directly above the commitment checks.
  - **Title and outcome share the remaining width** (`1fr` and `1.35fr`) rather than the title taking the commitment grid's `1fr` and the outcome taking three fixed columns. Their left edges still align with the description column below; their right edges do not, and that is the trade: an outcome is a sentence and needs the room more than a title does. The empty-state prompt also dropped from `rows={3}` to `rows={1}` — `useAutoResize` grows it the moment anything is typed, and three blank rows made every outcome-less issue twice the height of its own title field.
  - **Seven columns, not eight.** Every commitment row on this surface is `hidePriority`, so `.rowNoPriority` is the grid to agree with. `.issueRow` and `.columnHeader` briefly carried eight, which put column 4's *start* in the right place but made its *width* different from the description column below it, because 180px more of fixed columns followed it. *What we want* spans `5 / -1`.
  - **One column system for the whole card.** `.issueRow` and `.columnHeader` carry the SAME columns a `CommitmentRow` does, and the issue's cells are placed into them explicitly (drag at 1, title at 4 over the commitment descriptions it heads, *what we want* spanning 5–6, delete at 7, resolve at 8). Two unrelated grids stacked on each other read as two tables that happened to land in one card. The mobile breakpoint releases every placement with `grid-column: auto`, because down there the row is a single column.
  - **Source order is load-bearing.** The commitments region spans all eight columns, so anything declared after it is auto-placed onto a *third* row. Delete and Resolve were, and appeared stranded on their own line under the issue. They are declared before the region now.
  - **The commitments region is a recessed panel.** An issue and the commitments under it were otherwise one undifferentiated field of white. It uses the two tones the app already has rather than a new colour — cards are `--surface`, the page behind them is `--bg`, so a sand panel inside a white row reads as inset. Deliberately NOT indented: the card shares one column system, and a left inset would buy the visual cue by knocking every commitment column out of line with the header above it. Tone carries the hierarchy; the columns stay honest. Issue-to-issue separation moved up to `--border-strong` so the eye reads "next issue" rather than "next row".
  - **`.resolveButton` and `.reviewResolve` use the shared button system.** `.resolveButton` had hand-rolled its own padding, height, radius, weight, hover and focus ring — a ninth variant that had drifted from the ~25 call sites composing `.btn*` from `ui.module.css`. It is now `btnGhost btnSm`; `.reviewResolve` composed `btnPrimary` and then re-specified padding and font by hand, which is `btnSm` spelled out, so it uses that.
  - **Each issue labels its own commitment block.** The list header above the cards is now ISSUE | WHAT WE WANT only; `CommitmentSubHeader` inside each card `composes` the `/commitments` `.row` grid so its labels sit over the real columns rather than a copy that can drift. The add line composes the same grid, putting its plus in the resolve-circle column.
  - **The add form is the last line, always present,** so a first and a fifth commitment are entered the same way.
  - **This replaced a hierarchy that was never in the domain.** The first version gave one commitment the row's three cells and rendered the rest as second-class lines underneath, because the grid had exactly three commitment-shaped columns. That asymmetry produced three defects in a single day: an unreachable add affordance, a second commitment displacing the first into a collapsed panel, and a review prompt wedged into a cell too narrow for it. There is no "active" commitment any more.
  - **Commitments can be closed from the issue.** Previously there was no way to resolve an issue commitment on this page at all, which sent people to `/commitments` to finish work they were looking straight at. The first fix added a one-click check; it is gone, because one click with no menu and no undo was a *different* contract from the same glyph on `/commitments`.
  - **The clarity dot moved onto the line.** Clarity belongs to a commitment, and an issue has any number of them, so the row's leading clarity column is gone. Its placement is now whatever `CommitmentRow` does, which is beside the resolve circle.
  - **The issue's resolve control is the commitment resolve circle,** composed from `commitments.module.css` rather than imitated, and it sits in column 1 with delete at 2 and the drag handle at 3 — the same control columns a commitment line uses. It was a pill labelled *Resolve* in the LAST column, which made acting on an issue look like a different kind of operation from acting on a commitment and put it at the far end of the row from the controls it belongs with. No menu: an issue cannot be parked or rescheduled, so a menu of one item is a click in the way. It still never resolves on the click — the existing `ConfirmDialog` is the second gesture, which is what the commitment circle's menu is doing too. A resolved issue carries a static filled check in the same column (`.issueResolvedMark`), composed from the same circle but deliberately WITHOUT `resolveCircleChecked`, whose pop animation is right for the moment you resolve something and wrong for forty rows of history rendering at once. Static, not a button, because there is no reopen and an affordance would promise one.
  - **The add line and sub-header compose `rowNoPriority`, not `row`.** `hidePriority` does not renumber anything: `.rowNoPriority` is a SEVEN-column template that hides the sixth CHILD. Composing the eight-column `.row` and `.columnHeader` put the add line's fields and every header label from ASSIGNED TO rightward two columns away from the data they belonged to. Both now render eight children against seven columns, exactly as `CommitmentRow` does, so the priority spacer must be present in order to be hidden.
  - **Adding takes a button; editing does not.** The add line ends with the same filled, labelled `Add` submit `/commitments` has always had, disabled until there is something to add. Editing an existing commitment is blur-to-save inline, inherited rather than implemented: `CommitmentRow`'s description textarea carries `onBlur={commitDescription}` and the owner picker commits on change. Reuse is what makes the two pages agree here.
  - **The add line's plus is gone.** It was a filled blue circle holding a bare `+`, sitting in the RESOLVE-CIRCLE column, where it read as that row's resolve control. Asked what it did, nobody could say — fair, because a `+` does not say. Column 1 on the add line is now empty: there is no commitment on that line yet, so there is nothing to resolve.
  - **Superseded: the plus as a button that always does something.** With text in the field it submits; empty, it focuses the field. It was briefly a `<label>`, which focuses its control natively and adds no tab stop — correct, and it read as broken, because clicking it on an empty line moved the caret into a box that was already visible and already showing its placeholder, so nothing appeared to happen. An affordance that looks like a button has to answer a click visibly, so `.addLine:focus-within` lights the line and the circle carries hover, active and focus states. The circle uses `--primary` / `--on-primary` rather than the raw brand hexes because the dark theme redefines the pair together.
  - **`.addLine` does not compose `.cellCommitment`.** That class is `display: -webkit-box` with a three-line clamp. Composing it made the add form a clamping container rather than a flex row, so `flex-direction: row` was inert and the plus rendered *above* the textarea.
  - **`e2e/issue-thread.spec.ts` drives both gestures.** Resolving is deliberately no longer one click, so the spec opens the circle's menu and chooses *Mark kept*. It also presses Cmd/Ctrl+Enter on the add field instead of clicking an add button, which the add line has never had. The e2e suite is not one of the four CI gates.
- **Finished commitments collapse behind `N done`.** The one place the uniform rule bends, and deliberately: an issue with eight finished commitments would otherwise bury the live ones. Expanding renders them as the same `CommitmentRow` lines everything else uses, which means they are NOT read-only: a finished commitment offers *Reopen* from its circle menu exactly as it does on `/commitments`. That is a change — the earlier bespoke lines were read-only on the reasoning that reopening belonged to the surface where a commitment is still live. Reuse settled it the other way, and consistently: there is now one answer to "what can I do to a commitment", not one per page. The current open commitment keeps the slot it has always had. An issue with no history is byte-for-byte the row it was before the thread existed — no marker, no badge, no toggle.
- **The review moment.** When every commitment on an unresolved issue has landed, a single line at the end of the thread asks **"Did this solve it?"** with *Resolve issue* beside it and a note pointing at the add line below. It spans all three columns because it is a question about the **issue**, not about a commitment. **Nothing is automatic** — no auto-resolve, no auto-created follow-up. A quiet `needs review` badge marks the same state in the list.
...
- **The resolved list is the open list in past tense** (2026-09-14). Same grid, same `CommitmentSubHeader`, same `CommitmentRow` lines. `.resolvedRow` and `.resolvedColumnHeader` are now one-line `composes` of `.issueRow` and `.columnHeader`; the four hand-maintained grid templates they replaced (a five-column row, a copy of it with a trailing 28px delete cell, two matching headers) and their duplicate mobile-collapse rules are gone, along with the `isAdmin` class-swapping that chose between them. The delete slot is always in the grid and the row renders a placeholder into it, which is what the open row always did.
  - **It shows every commitment, not a representative one.** It used to render the single newest linked commitment in a cell, so an issue that took four attempts displayed the fourth and silently dropped three. On a surface whose entire premise is that an issue takes more than one go, the history was the part being hidden. `resolvedCommitmentCell` survives unchanged and still decides what to show when NO commitment carries text: *Resolved in meeting* for the meeting-summary shortcut, a plain statement otherwise.
  - **Collapsed behind a disclosure, one line per issue** (2026-09-14). Rendering every resolved issue's full commitment block gave the archive several stacked blocks of equal weight with no way to tell where one issue ended and the next began. The thread is the record of how an issue got solved — worth keeping, worth reading occasionally, not worth rendering in full for every row. A **chevron** rather than a plus: on this page a filled circle holding a plus is the Add control on the commitment line, and a second meaning for the same glyph two rows away is how the first one stopped being readable. It sits in column 4, the empty slot immediately left of the title it opens; an issue that never carried a commitment renders a placeholder there so every title still starts in the same place.
  - **No `N done` sub-collapse inside an expanded thread.** That exists on an open issue so finished work cannot bury live work. A resolved issue has no live work to bury.
- **Resolving an issue closes the commitments on it** (2026-09-14). They resolve as **kept** — on time or late against their own due date, the same rule `markKeptAction` applies when an owner ticks one by hand — with `resolved_by_role` and `resolved_by_profile_id` recording who did it.
  - **This writes real performance data** into Follow-Through and the weekly scorecard, which is why the confirm dialog names the count and says so: *"all 3 commitments still open on it will be marked kept. That counts toward follow-through, so only resolve if the work actually landed."* The person clicking is the one asserting the work is done. The dialog itself stays, against the general rule that confirms guard destructive actions only — it is the thing that makes the assertion informed.
  - **Two exclusions.** *Parked* rows are already outside every metric; closing one would drag it back in and claim it was kept. *Ongoing* rows never close — they record an occurrence and roll forward — and rolling somebody's recurring commitment forward as a side effect of resolving an issue is not what "close this" means. The dialog's count excludes both, so it never promises more than the action does.
  - **RLS is the boundary.** The cascade runs on the caller's client, so a non-admin issue owner resolving an issue that carries somebody else's commitment closes their own and leaves the other; the database refuses it, and the returned count reflects what actually changed.
  - **Reopening one afterwards is fine and changes nothing about the issue.** `unmarkKeptAction` writes only to the commitment. The issue stays resolved and the commitment goes live again — the same state the six grandfathered issues are already in, which the thread model was built to tolerate.
- **There is no reopen** *for an issue*. Nothing writes `issues.status` back to `open` after creation. If the review question is answered "not solved" on an already-resolved issue, today's answer is a new issue. A reopen path is a future decision, not an omission.
...
- **Function roles** — beyond LTD, function nodes carry named leadership archetypes (Visionary, Integrator, etc.) via `function_roles` for the strategic top of the chart.
  - **The baseline row is immutable.** Each function carries one `is_default` row holding "Lead, Track, Decide", protected by RLS from update and delete and by a partial unique index from being duplicated. Until migration 0193 that was true of `system_admin` and `company_admin` and **not** of `aims_guide`: the guide policies added in 0111 mirrored the tenant predicate and dropped the `is_default` clause, so a guide could edit and delete the row a system admin could not. Nothing in the app reached it — every write path carries `.eq("is_default", false)` itself — and it was found by probing the guard as the third role rather than assuming two refusals meant three. INSERT is deliberately left unrestricted on `is_default`; the partial unique index prevents a second baseline per function, and whether one should be creatable at all outside the 0107 seed is its own question.
...
- **Apply-to-Chart** (`src/lib/chart/apply-proposal-action.ts`) — additive-only writes into the existing chart tables (`functions` + `function_roles`) via the admin Supabase client, with the app-layer `isAdminForCompany` check as the security boundary. *(The parenthetical here used to justify that by saying guides hold no RLS insert rights on `function_roles`. They do — `function_roles_insert_guide` has existed since 0111. The admin client is still what this path uses, so the app-layer check is still the boundary; the reason given for it was simply wrong.)* Semantics:
...
- **Isolation:** every table has RLS enabled + `force row level security`; company-scoped policies check both role and company match. Cross-tenant reads flow through `auth_profile()` and `company_has_feature()` helper functions.
  - **RLS is not a backstop on service-role paths.** Anything routed through `createSupabaseAdminClient()` bypasses RLS entirely, so every such call site must check company membership itself.
    - **Most of the transcript surface is no longer one of them.** Migration 0181 gave `transcript_sources` and `transcript_aliases` real policies for company admins and assigned guides, and eight writes in `src/lib/transcripts/actions.ts` moved onto the caller's own client (`db()`), where RLS filters them as that person. Those writes check ROWS AFFECTED rather than `error`, because RLS refuses an UPDATE by matching nothing and returns no error at all. The `isAdminForCompany` guards stayed as well: app guards are courtesy, RLS is the boundary, and having both is the point.
    - **What is still service-role, and why.** Dismissing an unrouted meeting (`dismissMeetingAction`) — an unrouted row has no company, so no company-scoped policy can admit anybody, and the guard is genuinely the boundary. The `transcript_source_audit_log` insert — the table grants INSERT to no role by design, so only a service-role writer can append to it. The Google Drive ingest pipeline and the OAuth credential lookups, which run outside any user session. The OAuth callback re-checks membership on the company id parsed from the state cookie before persisting a credential.
...
- **Timezone:** per-company (`companies.timezone`). Week-ending Friday is computed in the company's local time. AI cron jobs run on a UTC schedule but resolve week boundaries per tenant.
  - **Editable by `system_admin` and `portfolio_admin` only, and every change is recorded** (migrations 0189, 0192). The zone is set on the company settings page from a fixed list (`src/lib/companies/timezones.ts`); a value off that list is refused rather than normalised. Company admins and guides cannot reach the column: 0176's column guard allows them `industry` and nothing else, and `timezone` is in the four-column allowlist 0192 gives the portfolio role. The harness probes both the grant and the two refusals as those roles rather than assuming them. Changing the zone does not migrate anything, it re-asks the question, so counts either side of the old midnight read differently afterwards. An append-only `company_settings_events` log records what moved, from what to what, when, and by whom, written by a database trigger so no write path can skip it, and the last three timezone changes render under the control that causes them. The log covers every column on `companies`, not a watchlist, and starts at 0189: nothing recorded what a timezone used to be before then.
...
- **Request-path cost.** Deliberate choices, all from the 2026-09-02 performance audit:
  ...
  - **RLS helpers are hoisted** (migrations 0175-0188, the F8 series, shipped 2026-09-13). Every policy that calls a helper wraps it as `(select public.auth_role())` — a scalar subquery with no outer reference, which Postgres evaluates once per statement as an InitPlan instead of once per row. A bare wrapper was measured 374x slower than the hoisted form, so the shape matters rather than merely the intent. **0 of the 373 live policies on each instance still call `auth_profile()` per row.** The one per-row exception is deliberate: `is_guide_for(company_id)` takes a per-row argument, so it cannot be an InitPlan and is not treated as one. `is_portfolio_admin()` takes none, which is exactly why the instance-wide read grant in 0191 costs one InitPlan per statement across all forty-nine of its policies rather than a call per row. `docs/f8-rls-hoist.md` records the shapes. Three latent "more than one row" hazards were found and removed on the way.
  - **The dashboard and layout loaders fan out.** `getDashboardData` runs two waves of `Promise.all` rather than thirteen sequential awaits, and the app layout resolves the scoped company once and then fetches the company row and its entitlements together. Both were listed here as deferred and both shipped.
