...
- **Every open commitment is always visible.** The row has one commitment slot, so the newest open one sits in it and any others render as thread lines carrying their own owner and due date. When more than one is open the thread is **not collapsible** — history can be folded away, live work cannot. Between 2026-09-14 and the fix later that day a second commitment displaced the first into a collapsed panel, so an issue with two open commitments looked like it had one.
- **The thread.** Finished commitments render as single collapsed lines, oldest first, behind a marker under the current commitment: `N done` where there is history, `+ add commitment` where there is not. Both open the same panel, which ends in the add form — so a second commitment can be added **alongside** one already in flight, not only after it lands. A brand-new issue with no commitment at all shows no marker: the row already carries an inline add form, and two would be one too many. The current open commitment keeps the slot it has always had. An issue with no history is byte-for-byte the row it was before the thread existed — no marker, no badge, no toggle.
