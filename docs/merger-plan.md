...
- [ ] **Phase 5 — Physical SM code import + module API/component merge** (sub-phased so nothing lands as one giant blob)
  ...
  - [ ] **5b — Lib import.** Port `src/lib/scoring.ts`, `src/lib/team-scoring.ts`, `src/lib/voice-rules.ts`, `src/lib/tenure.ts` and related into `src/lib/strengths/*`.
  - [ ] **5c — Assessment routes.** `/assessment`, `/results` pages + components under `/strengths/*`.
  - [ ] **5d — Team Builder.** SM's `/admin/teams/*` under `/strengths/teams/*`.
  - [ ] **5e — API endpoints.** `/api/narrative`, `/api/generate-results`, `/api/generate-team-insights` → `/api/strengths/*`.
  - [ ] **5f — Coach entry-point wiring.** SM's coach page hits our unified `/api/coach` with `contextKind: 'strengths'`; SM's old inline `/api/coach` implementation deleted.
  - [ ] **5g — Nav strengths items.** Add `/strengths/*` links to `APP_LINKS` with `feature: 'strengths'`; they auto-gate on the subscription set up in Phase 1.
  - [ ] **5h — Wire strengths context assembler.** Replace the Phase 4 stub in `buildCoachContext` with real queries against `strengths_results` + `strengths_narrative_messages`.
